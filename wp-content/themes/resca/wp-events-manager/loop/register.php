...
if ( absint( $event->qty ) == 0 || $event->post->post_status === 'tp-event-expired' || get_post_meta( get_the_ID(), 'tp_event_status', true ) === 'expired' ) {
	return;
}
	...
	<div class="register-inner">
		...
		<?php if ( ! is_user_logged_in() ) { ?>
			<span class="inner-notice">
				<?php
				printf( wp_kses( __( '<span class="event_auth_user_is_not_login">You must <a href="%s">Login</a> to Our site to register this event!</span>', 'resca' ), array( 'a'  => array( 'href' => array() ),
				                                                                                                                                                                'br' => array()
