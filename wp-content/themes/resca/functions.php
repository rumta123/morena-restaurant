...
if ( is_multisite() ) {
	...
	define( 'RESCA_UPLOADS_FOLDER', trailingslashit( WP_CONTENT_DIR ) . 'uploads/sites/' . get_current_blog_id() . '/tc_stylesheets/' );
	define( 'RESCA_UPLOADS_URL', trailingslashit( WP_CONTENT_URL ) . 'uploads/sites/' . get_current_blog_id() . '/tc_stylesheets/' );
} else {
	...
	if ( !defined( 'RESCA_UPLOADS_FOLDER' ) ) {
		define( 'RESCA_UPLOADS_FOLDER', trailingslashit( WP_CONTENT_DIR ) . 'uploads/tc_stylesheets/' );
	}

	if ( !defined( 'RESCA_UPLOADS_URL' ) ) {
		define( 'RESCA_UPLOADS_URL', trailingslashit( WP_CONTENT_URL ) . 'uploads/tc_stylesheets/' );
	}
}

if ( !defined( 'RESCA_FILE_NAME' ) ) {
	define( 'RESCA_FILE_NAME', 'resca-options.css' );
}
...
function thim_scripts() {
	...
	if ( is_file( RESCA_UPLOADS_FOLDER . RESCA_FILE_NAME ) ) {
		wp_enqueue_style( 'resca-options', RESCA_UPLOADS_URL . RESCA_FILE_NAME, array(), filemtime( RESCA_UPLOADS_FOLDER . RESCA_FILE_NAME ) );
	} else {
		wp_enqueue_style( 'resca-options', get_template_directory_uri() . '/assets/css/resca-options.css', array(), TP_THEME_VERSION );
	}
...
// import demo
require TP_THEME_DIR . 'inc/admin/demo-data.php';
