...
} ?>" <?php echo ent2ncr( $c_css ); ?> data-parallax_images="scroll" data-image-src="<?php echo esc_url( $cate_top_image_src ); ?>">
				...
				if ( ( is_page() || is_single() ) && get_post_type() != 'product' ) {
					if ( is_single() ) {
						if ( get_post_type() == "portfolio" ) {
							...
							if ( ! empty( $category ) ) {
								$array_value = array_values( $category );
								// Get last category post is in
								$last_category = end( $array_value );
